    ...
    class SshCisco < SSHBase
      ...
      def converge(state) # rubocop:disable Metrics/AbcSize
        ...
        instance.transport.connection(backcompat_merged_state(state)) do |conn|
          ...
          info("SshCisco Transferring files to #{instance.to_str}")
          conn.upload(sandbox_dirs, provisioner[:root_path])
          debug("SshCisco Transfer complete")
      ...
      def create(state)
        ...
        print "SshCisco does not start your server '#{state[:hostname]}' but will look for an ssh connection with user '#{state[:username]}'"
        wait_for_sshd(state[:hostname], state[:username], {:port => state[:port]})
        print "SshCisco found ssh ready on host '#{state[:hostname]}' with user '#{state[:username]}'\n"
        debug("SshCisco:create '#{state[:hostname]}'")
      end

      def destroy(state)
        print "SshCisco does not destroy your server '#{state[:hostname]}' by shutting it down..."
        ...
        debug("SshCisco:destroy '#{state[:hostname]}'")
      end
